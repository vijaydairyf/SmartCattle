...
namespace SmartCattle.Web.Controllers
{
    public class NotificationController : Controller
    {
        ...
        [AuthenticateFilter]
        public String SaveAction(String ID, String Msg, String User)
        {
            String ret = "NaN";
            int tmpID = Convert.ToInt32(ID.Replace("RowId", ""));
            string _user = Helper.Helper.getCurrentUserEmail();
            User = _user;
            if (User != "")
            {
                ISession mContext = Context.Open();
                IList<UserInfo> currentUser = mContext.QueryOver<UserInfo>().Where(x => x.Email == User).Where(x => x.FarmId == Helper.Helper.getCurrentFarmId()).List<UserInfo>();
                Context.Close(mContext);
        ...
        [AuthenticateFilter]
        public String DeactiveAction(String ID, String User)
        {
            String ret = "NaN";
            int tmpID = Convert.ToInt32(ID.Replace("RowId", ""));
            string _user = Helper.Helper.getCurrentUserEmail();
        ...
        [AuthenticateFilter]
        public String ActiveAgainAction(String ID, String User)
        {
            User = User.Replace("\n", "").Replace(" ", "");
            String ret = "NaN";
            int tmpID = Convert.ToInt32(ID.Replace("RowId", ""));
